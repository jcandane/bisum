...
@torch.jit.script
def spa_post_trans(a_index, a_data, a_shape, rhs, RHS):
    ...
    cc= a_index[m,:]
    s = torch.tensor(a_shape)[m] ## permute shape...
    n = lexsort( cc )
    return [ cc[:,n] , a_data[n], [int(i.item()) for i in s] ]

@torch.jit.script
def spa_post_intraTr(a_index, a_data, a_shape, label):

    uniques, counts = torch.unique(label, return_counts=True)
    if torch.all(counts==1): ## no duplicates in label
        return [ a_index, a_data, a_shape ]
        ...
        if torch.numel(index)!=0: ## index is empty
            ...
            ZZ      = torch.zeros_like(index[0], dtype=a_data.dtype)
            data    = ZZ.scatter_add(0, domains, data)

            return [ index, data, size ] #, label
        else:
            data = torch.unsqueeze(torch.unsqueeze(data, 0),0)
            return [ torch.zeros_like(data[:,:,0], dtype=torch.int64), torch.sum(data), [int(b) for b in torch.ones_like(data[:,0,0])] ]


@torch.jit.script
def spa_tensor_intraTr(a_index, a_data, a_shape, label, intratrace):
